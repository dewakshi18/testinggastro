...
import { MOCK_BED_RATES } from '@/mockData';
...
export default function PatientOverview({ userRole }: { userRole?: string }) {
  ...
  const assignedPatientIds = useMemo(() => {
    ...
    allAppts.forEach((apt: any) => {
      ...
      const isDocMatch = 
        ...
        (aptDocName && currentUserNameStr.includes(aptDocNameLower)) ||
        (currentUser.name && aptDocNameLower.includes(currentUserNameStr));
  ...
  const fetchPatientDetails = async (id: string) => {
    const [appts, bills, rx, vts, labs, claims, rads, dels, babies, notes, adms] = await Promise.all([
    ...
    if (adms) setAdmissions(adms.filter((a: any) => a.patient_id === id || a.patientId === id));
    if (bills) {
      const patientBills = bills.filter((b: any) => b.patient_id === id || b.patientId === id);
