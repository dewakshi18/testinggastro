...
import { MOCK_USERS, MOCK_PATIENTS, MOCK_BED_RATES, MOCK_OT_RATES, MOCK_LAB_TESTS, MOCK_MATERIAL_RATES, MOCK_ENDO_RATES } from '@/mockData';
...
export default function Settings({ currentUser, onUserUpdate, onHospitalUpdate }: { currentUser?: any, onUserUpdate?: (user: any) => void, onHospitalUpdate?: (info: any) => void }) {
  ...
  const [hospitalInfo, setHospitalInfo] = useState(() => storage.get(STORAGE_KEYS.HOSPITAL_INFO, {
    ...
    gst: '27AAAAA0000A1Z5',
    phone: '+918601561055',
    email: 'info@neogastroplushospital.com',
  ...
  }, [bedRates, otRates, labRates, materialRates, endoRates, opdCharges, defaultTokenSize]);
      ...
      <Tabs defaultValue="hospital" className="space-y-6">
        ...
        <TabsContent value="rates">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Endoscopy & GI Procedure Rate Master */}
            <Card className="border-none shadow-sm lg:col-span-2">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <CardTitle>Endoscopy & GI Procedure Rate Master</CardTitle>
                      <CardDescription>Admin configuration for Base Procedure Fees, Sedation Fees, and Biopsy Pack Fees for Endoscopy/Colonoscopy suite.</CardDescription>
                    </div>
                  </div>
                  <Badge variant="outline" className="bg-sky-50 text-sky-700 border-sky-200 text-xs font-bold">Admin Rate Master</Badge>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                {!isAccountant && (
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
                    <p className="text-xs font-bold text-slate-700 uppercase tracking-wider">Add / Customize Procedure Rate Package</p>
                    <div className="grid grid-cols-1 sm:grid-cols-5 gap-2">
                      <Input 
                        placeholder="Procedure Name (e.g. Diagnostic EGD)" 
                        value={newEndoProcedure.name} 
                        onChange={(e) => setNewEndoProcedure({...newEndoProcedure, name: e.target.value})} 
                        className="sm:col-span-2 text-xs bg-white"
                      />
                      <Input 
                        type="number" 
                        placeholder="Base Fee (₹)" 
                        value={newEndoProcedure.baseFee} 
                        onChange={(e) => setNewEndoProcedure({...newEndoProcedure, baseFee: e.target.value})} 
                        className="text-xs bg-white"
                      />
                      <Input 
                        type="number" 
                        placeholder="Sedation Fee (₹)" 
                        value={newEndoProcedure.sedationFee} 
                        onChange={(e) => setNewEndoProcedure({...newEndoProcedure, sedationFee: e.target.value})} 
                        className="text-xs bg-white"
                      />
                      <Button 
                        className="bg-sky-600 hover:bg-sky-700 text-white font-bold text-xs gap-1"
                        onClick={() => {
                          if (!newEndoProcedure.name.trim()) {
                            toast.error('Please enter a valid procedure name');
                            return;
                          }
                          const updated = {
                            ...endoRates,
                            [newEndoProcedure.name.trim()]: {
                              baseFee: parseFloat(newEndoProcedure.baseFee) || 0,
                              sedationFee: parseFloat(newEndoProcedure.sedationFee) || 0,
                              kitFee: parseFloat(newEndoProcedure.kitFee) || 0,
                              category: 'Endoscopy'
                            }
                          };
                          setEndoRates(updated);
                          setNewEndoProcedure({ name: '', baseFee: '', sedationFee: '0', kitFee: '0', category: 'Endoscopy' });
                          toast.success(`Procedure "${newEndoProcedure.name}" rate card saved!`);
                        }}
                      >
                ...
                <div className="space-y-2">
                  <div className="grid grid-cols-12 text-[10px] font-extrabold uppercase text-slate-400 px-3 py-1">
                    <div className="col-span-5">Procedure Package Name</div>
                    <div className="col-span-2 text-right">Base Fee (₹)</div>
                    <div className="col-span-2 text-right">Sedation Fee (₹)</div>
                    <div className="col-span-2 text-right">Biopsy Fee (₹)</div>
                    <div className="col-span-1 text-center">Actions</div>
                  </div>
                  {Object.entries(endoRates).map(([pName, rate]: [string, EndoRateItem]) => (
                    <div key={pName} className="grid grid-cols-12 items-center p-3 bg-slate-50 rounded-xl border border-slate-100 gap-2">
                      <div className="col-span-5">
                        <p className="text-xs font-bold text-slate-900">{pName}</p>
                        <span className="text-[10px] text-slate-400 font-semibold">{rate.category || 'GI Procedure'}</span>
                      </div>
                      <div className="col-span-2">
                        <Input 
                          ...
                          onChange={(e) => {
                            ...
                            setEndoRates(prev => ({
                              ...prev,
                              [pName]: { ...prev[pName], baseFee: val }
                            }));
                          }}
                          className="h-8 text-xs text-right font-bold bg-white border-slate-200"
                        />
                      </div>
                      <div className="col-span-2">
                        <Input 
                          type="number"
                          value={rate.sedationFee}
                          onChange={(e) => {
                            const val = parseFloat(e.target.value) || 0;
                            setEndoRates(prev => ({
                              ...prev,
                              [pName]: { ...prev[pName], sedationFee: val }
                            }));
                          }}
                      ...
                      <div className="col-span-1 flex justify-center">
                        {!isAccountant && (
                          <Button 
                            ...
                            onClick={() => {
                              const updated = { ...endoRates };
                              delete updated[pName];
                              setEndoRates(updated);
                              toast.success(`Removed rate card for "${pName}"`);
                            }}
                          >
            ...
            <Card className="border-none shadow-sm lg:col-span-2">
              <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div>
                  ...
                  <CardDescription>Manage rates and sample vials for all diagnostic tests.</CardDescription>
                </div>
                {!isAccountant && (
                    ...
                    {labRates.map((rate: any) => (
                      <div key={rate.id || rate.name} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100">
                        <div>
                          ...
                          {rate.vial && (
                            <Badge variant="secondary" className="ml-1.5 text-[9px] bg-indigo-50 text-indigo-700 font-semibold border-indigo-100">
                              Vial: {rate.vial}
                            </Badge>
                          )}
                        </div>
