...
const DEFAULT_AOR_RECORD: AnaestheticOpRecordData = {
  hospitalName: 'NEO GASTRO HOSPITAL',
  regLicenseNo: 'CMHO Reg. & License No.: NH327/DEC/2027',
  docNo: 'DOC. No. NGH/OT/AOR/15/Ver. 1.0/2026',
...
export default function AnaestheticOperationRecord({ patientData, onSave }: AnaestheticOperationRecordProps) {
  const [record, setRecord] = useState<AnaestheticOpRecordData>(() => {
    ...
    const hospName = savedHosp?.name || 'NEO GASTRO HOSPITAL';
    const hospAddr = savedHosp?.address || 'Infront of Aura Inn, Bansi Road near Badewan, Basti UP 272001';
  ...
  const handlePrint = (printBlank = false) => {
    ...
    const savedHosp = storage.get(STORAGE_KEYS.HOSPITAL_INFO, null);
    const hospName = savedHosp?.name || record.hospitalName || 'NEO GASTRO HOSPITAL';
    const hospAddr = savedHosp?.address || 'Infront of Aura Inn, Bansi Road near Badewan, Basti UP 272001';
    const hospPhone = savedHosp?.phone || '+91-8601561055';
