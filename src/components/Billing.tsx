...
import { formatCurrency, formatDate, buildDateWiseInvoiceMap, buildSequentialInvoiceMap, buildDepartmentWiseInvoiceMap, getCleanDateString } from '@/lib/utils';
...
export default function Billing() {
  ...
  const getBillPatientInfo = (bill: any) => {
    if (!bill) return { id: '', name: 'Registered Patient', mrn: 'N/A', phone: 'N/A', email: '', isMatched: false, department: 'General' };
    ...
    const nameMatch = !directMatch && pName ? 
      patients.find(p => p.name && p.name.toLowerCase().trim() === pName.toLowerCase().trim()) : null;
    const phoneMatch = !directMatch && !nameMatch && pPhone && pPhone !== 'N/A' && pPhone !== '0000000000' ?
      patients.find(p => p.phone === pPhone || p.mobile === pPhone) : null;
    ...
    const deptType = bill.type || (bill.items?.[0]?.category) || 'General';
    ...
    const phone = matched?.phone || matchedApt?.patientPhone || bill.patients?.phone || bill.patient_phone || bill.patientPhone || bill.phone || 'N/A';
  ...
  const getServicesByCategory = (catId: string) => {
    ...
    let services: { name: string; rate: number; isCustom?: boolean; id?: string }[] = [];
    ...
    if (catId === 'ot') {
      ...
      services = materialRates.map((t: any) => ({ name: t.name, rate: t.price }));
    } else if (catId === 'opd') {
      services = [{ name: 'OPD Consultation', rate: 500 }];
    } else if (catId === 'pharmacy') {
      services = [{ name: 'Pharmacy Bill', rate: 0 }];
    }
  ...
  const filteredBills = bills.filter(bill => {
    ...
    if (filterPaymentMethod !== 'all') {
      const bMethod = (bill.payment_method || bill.paymentMethod || '').toLowerCase();
          ...
          <div class="invoice-wrapper">
            ...
            <div class="hospital-header">
              ...
              <div class="hospital-contact-block">
                ...
                <div>Tel: ${hospitalInfo.phone} | Email: ${hospitalInfo.email || 'info@hospital.com'}</div>
                <div>Web: ${hospitalInfo.website || 'www.neogastroplus.com'}</div>
              </div>
            </div>
            ...
            <div class="info-grid">
              <div class="meta-block">
                ...
                <div class="meta-row"><span class="meta-label">Name:</span> <span class="meta-value" style="font-size: 13.5px;">${patient?.name || 'Walk-in Patient'}</span></div>
                <div class="meta-row"><span class="meta-label">MRN / ID:</span> <span class="meta-value">${patient?.mrn || patient?.id || 'N/A'}</span></div>
                <div class="meta-row"><span class="meta-label">Age / Gender:</span> <span class="meta-value">${patient?.age ? `${patient.age} Y` : 'N/A'} ${patient?.gender ? `/ ${patient.gender}` : ''}</span></div>
                <div class="meta-row"><span class="meta-label">Contact No:</span> <span class="meta-value">${patient?.phone || patient?.mobile || 'N/A'}</span></div>
              </div>
              <div class="meta-block">
                <div class="meta-heading">Invoice Details</div>
                <div class="meta-row"><span class="meta-label">Invoice No:</span> <span class="meta-value">#${activeInvoiceMap[bill.id] || sequentialIdMap[bill.id] || (bill.id ? bill.id.toUpperCase() : 'INV-001')}</span></div>
                ...
                <div class="meta-row"><span class="meta-label">Billing Type:</span> <span class="meta-value">${bill.type || (bill.items?.[0]?.category) || 'OPD / Hospital Services'}</span></div>
              </div>
            </div>
              ...
              <tbody>
                ${bill.items.map((item: any, idx: number) => `
                  <tr>
                    ...
                    <td>
                      <div class="service-cat">${item.category}</div>
                    </td>
                                ...
                                return (
                                  <div key={bill.id} className="flex items-center justify-between p-3 rounded-lg border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition-colors">
                                    <div className="flex items-center gap-4">
                                      <div className="text-xs font-bold text-medical-blue">#{sequentialIdMap[bill.id] || bill.id.split('-')[1]?.substring(0, 6) || bill.id.substring(bill.id.length-6)}</div>
                                      <div>
                                        <p className="text-sm font-semibold">{patInfo.name}</p>
