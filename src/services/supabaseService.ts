  ...
  while (attempt < maxAttempts) {
    try {
      ...
      if (!deletedKey) {
        ...
        const badKey = keys.find(k => /[A-Z]/.test(k) || ['temp', 'bp', 'rr'].includes(k));
        if (badKey) {
...
const rawSupabaseService = {
  ...
  getStaff: async () => {
    try {
      ...
      // Decoded arrays
      const decodedStaff = decodeHelper((sData || []).filter(s => !isPlaceholderProfile(s)));
      const decodedProfiles = decodeHelper((pData || []).filter(p => !isPlaceholderProfile(p)));

      // Merge maps by ID (case-insensitive)
      const mergedMap = new Map<string, any>();

      // 1. Seed base default doctors & hospital staff
      MOCK_USERS.forEach((u: any) => {
        if (u && u.id) {
          mergedMap.set(String(u.id).toLowerCase(), { ...u });
          if (u.name) {
            mergedMap.set(String(u.name).toLowerCase(), { ...u });
          }
        }
      });

      // 2. Merge local storage users if any exist
      const localUsers = storage.get(STORAGE_KEYS.USERS, []);
      if (Array.isArray(localUsers)) {
        localUsers.filter(u => !isPlaceholderProfile(u)).forEach((u: any) => {
          if (u && u.id) {
      ...
      // Deduplicate and filter out any remaining placeholders
      const uniqueList: any[] = [];
      const seenIds = new Set<string>();
      const seenNames = new Set<string>();

      Array.from(mergedMap.values()).forEach((member: any) => {
        if (!member || !member.name || isPlaceholderProfile(member)) return;
    ...
    try {
      ...
      if (dbProfile.role) {
        ...
        if (r === 'RECEPTION' || r === 'RECEPTION_STAFF' || r === 'RECEPTIONIST') {
          ...
          dbProfile.role = 'ACCOUNTANT';
        } else if (['SUPER_ADMIN', 'ADMIN', 'DOCTOR', 'SURGEON', 'NURSE', 'RECEPTIONIST', 'ACCOUNTANT', 'LAB_TECHNICIAN', 'PHARMACIST'].includes(r)) {
          dbProfile.role = r;
        } else {
          dbProfile.role = 'DOCTOR'; // Safe default fallback
        }
      }
      
      if (dbProfile.id) {
        dbProfile.id = isUuid(dbProfile.id) ? dbProfile.id : toDeterministicUuid(dbProfile.id);
      } else {
  ...
  updateStaff: async (id: string, updates: any) => {
    try {
      ...
      delete dbUpdates.id; // avoid updating ID primary key column
      ...
      if (dbUpdates.role) {
        ...
        if (r === 'RECEPTION' || r === 'RECEPTION_STAFF' || r === 'RECEPTIONIST') {
          ...
          dbUpdates.role = 'ACCOUNTANT';
        } else if (['SUPER_ADMIN', 'ADMIN', 'DOCTOR', 'SURGEON', 'NURSE', 'RECEPTIONIST', 'ACCOUNTANT', 'LAB_TECHNICIAN', 'PHARMACIST'].includes(r)) {
          dbUpdates.role = r;
        } else {
          dbUpdates.role = 'DOCTOR'; // Safe default fallback
        }
      }
      ...
      // Sync to local storage
      const existing = storage.get(STORAGE_KEYS.USERS, MOCK_USERS);
      const updatedList = existing.map((u: any) => (u.id === id || u.id === dbId) ? { ...u, ...result } : u);
      ...
      // Fallback
      const existing = storage.get(STORAGE_KEYS.USERS, MOCK_USERS);
      const updatedList = existing.map((u: any) => (u.id === id || u.id === dbId) ? { ...u, ...updates } : u);
      storage.set(STORAGE_KEYS.USERS, updatedList);
      return { id, ...updates };
    }
  },
  ...
  deleteStaff: async (id: string) => {
    try {
      ...
      // Sync to local storage
      const existing = storage.get(STORAGE_KEYS.USERS, MOCK_USERS);
      const filtered = existing.filter((u: any) => u.id !== id && u.id !== dbId);
